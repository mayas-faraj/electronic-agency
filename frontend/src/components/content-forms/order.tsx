...
import { WorkspacePremium, PermContactCalendar, LocalOffer, ReceiptLong } from "@mui/icons-material";
...
const Order: FunctionComponent<IOrderProps> = ({ id, onUpdate }) => {
  ...
  const action = async () => {
    const result = await getServerData(
      `query { order(id: ${id}) { id count totalPrice address note status isOfferRequest createdAt product { id } client { id email } offer { id } } }`
    );
  ...
  return (
    <ContentForm id={id} name="order" title="Order Info" command={orderCommand} onUpdate={() => action()}>
      ...
      <div className="column-double">
        ...
        <FormControl fullWidth margin="normal">
          <TextField
            ...
            label="Total price"
            value={info.isOfferRequest ? "Offer Request" : info.totalPrice}
          />
        </FormControl>
      </div>
      <FormControl fullWidth margin="normal">
      ...
      <div className="row-flex">
        <Button variant="contained" color="primary" onClick={() => setViewProduct(true)}>
          <LocalOffer /> Product Info
        </Button>
        <Button variant="contained" color="primary" onClick={() => setViewClient(true)}>
          <PermContactCalendar /> User Info
        </Button>
        <Button variant="contained" color="primary" onClick={() => setViewOffer(true)} disabled={!info.isOfferRequest}>
          <WorkspacePremium />
          {info.offerId === 0 ? "Create" : "Update"} Offer
        </Button>
        <Button variant="contained" color="primary" onClick={() => setViewReceipt(true)}>
