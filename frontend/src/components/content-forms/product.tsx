...
const Product: FunctionComponent<IProductProps> = ({ id, onUpdate }) => {
  ...
  const productCommand =
    id === undefined
      ? `mutation { createProduct(input: {subCategoryId: ${info.subCategoryId}, name: "${(info.name as string).trim()}", nameTranslated: "${(
          info.nameTranslated as string
        ).trim()}", model: "${(info.model as string).trim()}", image: "${info.image}", description: "${(info.description as string)?.replaceAll(
          "\n",
          "\\n"
          ...
          "\n",
          "\\n"
        )}", specificationTranslated: "${(info.specificationTranslated as string)?.replaceAll("\n", "\\n")}", specificationImage: "${info.specificationImage}", price: ${
          info.price !== "" ? info.price : null
        }, point: ${info.point !== "" ? info.point : null}, catalogFile: "${info.catalogFile}"}) { id name model } }`
      : `mutation { updateProduct(id: ${id}, input: {subCategoryId: ${info.subCategoryId}, name: "${(info.name as string).trim()}", nameTranslated: "${(
          info.nameTranslated as string
        ).trim()}", model: "${(info.model as string).trim()}", image: "${info.image}", description: "${(info.description as string)?.replaceAll(
          "\n",
          "\\n"
          ...
          info.price !== "" ? info.price : null
        }, point: ${info.point !== "" ? info.point : null}, catalogFile: "${info.catalogFile}", isDisabled: ${info.isDisabled}}) { id name model } }`;

  // load data function
  const action = async () => {
    if (id !== undefined) {
      const result = await getServerData(
        `query { product(id: ${id}) {id subCategory { id categoryId } name nameTranslated model image description descriptionTranslated specification specificationTranslated specificationImage catalogFile price point isDisabled} }`
      );
  ...
  React.useEffect(() => {
    ...
    const loadProduct = async () => {
      const result = await getServerData(
        `query { product(id: ${id}) {id subCategory { id categoryId } name nameTranslated model image description descriptionTranslated specification specificationTranslated specificationImage price point catalogFile isDisabled} }`
      );
      dispatch({ type: "set", key: "categoryId", value: result.data.product.subCategory.categoryId });
      dispatch({ type: "set", key: "subCategoryId", value: result.data.product.subCategory.id });
      dispatch({ type: "set", key: "name", value: result.data.product.name });
  ...
  return (
    <ContentForm
      ...
      commandDisabled={info.name === "" || info.nameTranslated === "" || info.model === "" || info.subCategoryId === 0}
      onUpdate={() => action()}
    >
      <div className="column-double">
        <ImageUpload
          ...
          formName="image"
          value={info.image as string}
          onChange={(url) => dispatch({ type: "set", key: "image", value: url })}
          token={StorageManager.get() ?? undefined}
        />
        <div>
          ...
          <FormControl fullWidth margin="normal">
            <TextField
              ...
              value={info.nameTranslated}
              onChange={(e) => dispatch({ type: "set", key: "nameTranslated", value: e.target.value })}
            />
          </FormControl>
          <FormControl fullWidth margin="normal">
          ...
          <div className="column-double">
            <FormControl fullWidth margin="normal">
              <TextField
                ...
                type="number"
                label="Price"
                value={info.price != null ? info.price : ""}
                onChange={(e) => dispatch({ type: "set", key: "price", value: e.target.value })}
              />
            </FormControl>
            <FormControl fullWidth margin="normal">
              <TextField
                ...
                type="number"
                label="Point"
                value={info.point != null ? info.point : ""}
                onChange={(e) => dispatch({ type: "set", key: "point", value: e.target.value })}
              />
            </FormControl>
          </div>
        </div>
      ...
      <div className="column-double">
        ...
        <FormControl fullWidth margin="normal">
          ...
          <Select
            ...
            onChange={(e) => dispatch({ type: "set", key: "subCategoryId", value: e.target.value })}
          >
      ...
      <FormControl fullWidth margin="normal">
        <TextField
          ...
          rows={5}
          label="Description"
          value={info.description != null ? info.description : ""}
      ...
      <FormControl fullWidth margin="normal">
        <TextField
          ...
          value={info.descriptionTranslated != null ? info.descriptionTranslated : ""}
          onChange={(e) => dispatch({ type: "set", key: "descriptionTranslated", value: e.target.value })}
        />
      </FormControl>
      <FormControl fullWidth margin="normal">
        <TextField
          ...
          rows={5}
          label="Specification"
          value={info.specification != null ? info.specification : ""}
          onChange={(e) => dispatch({ type: "set", key: "specification", value: e.target.value })}
        />
      </FormControl>
      <FormControl fullWidth margin="normal">
        <TextField
          ...
          multiline={true}
          rows={5}
          label="Specification (Arabic)"
          value={info.specificationTranslated != null ? info.specificationTranslated : ""}
          onChange={(e) => dispatch({ type: "set", key: "specificationTranslated", value: e.target.value })}
        />
      </FormControl>
      <div className="column-double">
        <ImageUpload
          ...
          uploadUrl="/upload-image"
          formName="image"
          value={info.specificationImage as string}
          onChange={(url) => dispatch({ type: "set", key: "specificationImage", value: url })}
          token={StorageManager.get() ?? undefined}
        />
        <div>
          <FormControl fullWidth margin="normal">
            <TextField
              variant="outlined"
              label="Catalog URL"
              value={info.catalogFile != null ? info.catalogFile : ""}
              onChange={(e) => dispatch({ type: "set", key: "catalogFile", value: e.target.value })}
            />
          </FormControl>
          {id !== undefined && (
            <FormControl fullWidth margin="normal">
